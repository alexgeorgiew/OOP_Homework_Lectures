...
public:
    ...
    Error();
    ~Error();
    Error(Error const& input);
    Error& operator=(Error const& input);
    void set_type(const ErrorType input);
    void set_message(const char* message);
private:
    ErrorType type;
    char* message;
    static char* Copy( const char* message);
    static int Lengthof( const char* message);
};